...
(function () {
  ...
  function init() {
    ...
    /* On a tall screen (iPad) the first headings sit inside the viewport at
       load, so they typed immediately — finished before anyone scrolled to
       them, leaving only the fade. Hold those until the first scroll (or a
       short grace period), so the typing happens where it can be seen. */
    var loadedAt = performance.now(), userScrolled = false;
    ...
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        ...
        if (e.isIntersecting) {
          ...
          h.__inView = true;
          if (!userScrolled && performance.now() - loadedAt < 600) {
            var gen = (h.__hold = (h.__hold || 0) + 1);
            var start = function(){ if (h.__hold === gen && h.__inView) typeIt(h); };
            window.addEventListener('scroll', function on(){ window.removeEventListener('scroll', on); start(); }, {passive:true});
            setTimeout(start, 2500);          /* nobody scrolled — type anyway */
            return;
          }
